...
import edu.stanford.zoha.mymaps.models.Place
...
class MainActivity : AppCompatActivity() {
    ...
    private fun filter(text: String?) {
        val temp: MutableList<UserMap> = ArrayList()
        for (d in userMaps) {
            //or use .equal(text) with you want equal match
            //use .toLowerCase() for better matches
            if (d.title.toLowerCase().contains(text.toString().toLowerCase())){
                Log.i(TAG, "I am here ${d.title}")
                temp.add(d)
            }
        }
        //update recyclerview
        mapAdapter.updateList(temp)
    }
    ...
    private fun showAlertDialogue(){
        ...
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener{
            val title = mapFormView.findViewById<EditText>(R.id.ettTitle).text.toString()
